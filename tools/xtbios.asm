...
_post:
    cli
    cld
    mov ax, 0xF000            ; we execute in F000
    mov ds, ax                ; temporary; reset below
    xor ax, ax
    mov ss, ax
...
; ---- build the interrupt vector table ------------------------------
    xor ax, ax
    mov es, ax                ; ES = 0  (IVT)
; default every vector to a harmless IRET
    mov di, 0
    mov cx, 256
.fill_ivt:
...
; INT 1E -> floppy disk parameter table (data pointer, not code)
    mov bx, 0x1E*4
    mov word [es:bx],   _floppy_dpt
    mov word [es:bx+2], 0xF000

; ---- bring up the 6845 / CGA in 80x25 text -------------------------
    call vid_init

; ---- DS now points at BDA for the rest of POST ---------------------
    mov ax, BDA
    mov ds, ax
    sti                       ; interrupts live

; ---- draw the Philips-style boot screen ----------------------------
    mov si, msg_ver
    call puts
    mov si, msg_model
    call puts
    ...
    mov si, msg_memfound
    call puts
    mov ax, [0x13]            ; base memory KB (640)
    call put_dec
    mov si, msg_3sp
    call puts
    mov ax, [0x13]
...
; ---- hand off to the bootstrap loader ------------------------------
    int 0x19
