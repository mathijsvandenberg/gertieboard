;==============================================================================
; irqtest.asm  --  bare-metal IRQ0 / timer interrupt diagnostic for gertieboard
;
...
entry:
        ...
        ; ---- program timer counter 0: mode 3, divisor 0 (= 65536) ----
        mov al, 0x36                ; ctr0, LSB+MSB, mode 3, binary
        out 0x43, al
        xor al, al
        out 0x40, al                ; LSB
        out 0x40, al                ; MSB -> divisor 0
...
;------------------------------------------------------------------------------
; 256 per-vector catch stubs (6 bytes each: push ax ; mov al,n ; jmp near common)
;------------------------------------------------------------------------------
stubs:
%assign v 0
%rep 256
